...
def walzeDrehen(walzeStandard, wp):
...
def plugboard():
    with open("plugboards/" + str(month) + '/' + str(day) + '.pkl', "rb") as f:
        plugboard = pickle.load(f)
    return plugboard


def encode_decode_char(walze1, walze2, walze3, umkehrwalze, wp1, wp2, wp3, b):
    walze1 = walzeDrehen(walze1, wp1)
    walze2 = walzeDrehen(walze2, wp2)
    walze3 = walzeDrehen(walze3, wp3)
    out = alphabet[plugboard().index(walze1.index(walze2.index(walze3.index(umkehrwalze[walze3[walze2[walze1[plugboard()[alphabet.index(b)]]]]]))))]
    return out


def encode_decode_text(walze1, walze2, walze3, umkehrwalze, wp1, wp2, wp3, text):
    out = ""
    for char in text:
        if alphabet.count(char) != 0:
            out = out + encode_decode_char(walze1, walze2, walze3, umkehrwalze, wp1, wp2, wp3, char)
            if wp1 == len(walze1) - 1:
                wp1 = 0
                if wp2 == len(walze2) - 1:
                    wp2 = 0
...
# ausführung
dateiOderText = input("Datei oder Text ver-/entschlüsseln? (d / t)")
if dateiOderText == "t" or dateiOderText == "T":
    print(encode_decode_text(walze1, walze2, walze3, umkehrwalze, wp1, wp2, wp3, input("Text eingeben: ")))
else:
    encode_decode_file(walze1, walze2, walze3, umkehrwalze, wp1, wp2, wp3)
a = input("Beenden")